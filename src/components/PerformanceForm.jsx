...
import { calculateAchievement, formatIndonesianDate } from "../utils/formatter";
import { 
  Calendar, 
  Building2, 
  Save, 
...
export default function PerformanceForm({ onReportSubmitted }) {
  ...
  useEffect(() => {
    ...
    KPI_METADATA.forEach(kpi => {
      ...
      const historyKpi = existingSubmission ? existingSubmission[kpi.key] : null;

      if (draftHasRealisasi) {
        // Prioritas 1: Draft lokal (user sedang mengedit)
        mergedValues[targetKey] = draft[targetKey] !== undefined && draft[targetKey] !== ""
          ? draft[targetKey]
          : (currentTargets[kpi.key] ?? 0);
        mergedValues[realisasiKey] = draft[realisasiKey] !== undefined ? draft[realisasiKey] : "";
      } else if (historyKpi) {
        // Prioritas 2: Data dari server/history (sudah pernah disubmit)
        mergedValues[targetKey] = historyKpi.target ?? (currentTargets[kpi.key] ?? 0);
  ...
  const handleResetForm = async () => {
    const isConfirmed = await confirm({
      ...
      message: "Apakah Anda yakin ingin mengosongkan semua isian form target & realisasi hari ini?",
  ...
  const getRealisasiClass = (val) => {
    const base = "w-13 text-center font-mono font-bold text-xs rounded-lg py-1 outline-none transition-all";
  ...
  const getAchStyle = (kpiKey, target, real) => {
    ...
    if (ach === null) return { color: "#94a3b8", bg: "transparent", pct: null, status: "" };

    const meta = KPI_METADATA.find(k => k.key === kpiKey);
    const lowerIsBetter = meta ? meta.lowerIsBetter : false;

    if (lowerIsBetter) {
      if (ach >= 100) return { color: "#10b981", bg: "rgba(16,185,129,0.09)", pct: ach, status: "Baik" };
      if (ach >= 75) return { color: "#f59e0b", bg: "rgba(245,158,11,0.09)", pct: ach, status: "Waspada" };
      return { color: "#ef4444", bg: "rgba(239,68,68,0.09)", pct: ach, status: "Buruk" };
    } else {
      if (ach >= 100) return { color: "#10b981", bg: "rgba(16,185,129,0.09)", pct: ach, status: "Tercapai" };
      if (ach >= 50) return { color: "#f59e0b", bg: "rgba(245,158,11,0.09)", pct: ach, status: "Mendekati" };
      return { color: "#ef4444", bg: "rgba(239,68,68,0.09)", pct: ach, status: "Kurang" };
    }
  };
  ...
  // Render responsive date title
  const formattedDateTitle = useMemo(() => {
    return formatIndonesianDate(activeDate);
  }, [activeDate]);

  // CSS Grid template matching LaporanPanel.jsx
  const colTemplate = "32px 1fr 72px 72px 56px";

  return (
    <div className="flex flex-col gap-5">
      
      {/* ══════════════════════════════════════════════
           01. PAGE TITLE & DATE (Matches FormatPreview.jsx)
      ══════════════════════════════════════════════ */}
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4">
          {isLoading && (
            <div className="flex items-center gap-1.5 text-xs text-sky-600 dark:text-sky-400 font-bold animate-pulse">
      ...
      ══════════════════════════════════════════════ */}
      <Card variant="solid" className="border-slate-200 dark:border-slate-800/60 shadow-lg">
        {/* Card Header */}
        <div className="px-4 py-4 border-b border-slate-200 dark:border-slate-800/60 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-xs sm:text-sm font-black text-slate-800 dark:text-slate-100 uppercase tracking-wide">Parameter Kinerja Distribusi</h3>
              <p className="text-[9px] font-medium text-slate-500 dark:text-slate-400 mt-0.5">
                Petugas: <span className="text-sky-600 dark:text-sky-400 font-semibold">{username || "Sistem"}</span>
              </p>
            </div>
          </div>
        ...
        >
          ...
          {isRealisasi && <div className="text-center">%</div>}
        </div>
        ...
        <div className="divide-y divide-slate-200/80 dark:divide-slate-800/30">
          {KPI_METADATA.map((kpi, idx) => {
            ...
            const fillWidth = targetVal > 0 && realisasiVal !== "" 
            ...
            return (
              ...
              >
                {/* 1. Kolom Nomor */}
                <div className="text-xs font-bold text-slate-400 dark:text-slate-600">{idx + 1}</div>
                ...
                <div className="pr-2.5">
                  <div className="flex items-center gap-1.5">
                    <div 
                      ...
                      style={{ backgroundColor: BULLET_COLORS[idx] }} 
                    />
                    <span className="text-xs font-semibold text-slate-750 dark:text-slate-200 leading-snug">
                      {kpi.label} <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium">({kpi.unit})</span>
                    </span>
                  </div>
                  
                  {/* Progress Bar (Hanya jika Realisasi & Target > 0) */}
                  {isRealisasi && realisasiVal !== "" && targetVal > 0 && (
                    <div className="ml-3 mt-1.5 h-1 bg-slate-200 dark:bg-slate-900 rounded-full overflow-hidden w-full max-w-[200px]">
                      <div 
                        className="h-full rounded-full transition-all duration-500 ease-out"
                        style={{ 
                          width: `${fillWidth}%`, 
                          backgroundColor: getBarColor(kpi.key, targetVal, realisasiVal) 
                        }} 
                      />
                    </div>
                  )}
                ...
                <div className="text-center flex justify-center">
                  {isRealisasi ? (
                    <input
                      ...
                      placeholder="-"
                      value={realisasiVal}
                      onChange={e => handleValChange(realisasiKey, e.target.value)}
                      className={getRealisasiClass(realisasiVal)}
                    />
                  ) : (
                    <span className="text-slate-650 dark:text-slate-600 font-mono font-bold text-xs select-none">-</span>
                  )}
                </div>

                {/* 5. Kolom Persentase (Hanya Realisasi) */}
                {isRealisasi && (
                  <div className="text-center flex justify-center">
                    {s.pct !== null && realisasiVal !== "" ? (
                      <span 
                      ...
                      <span className="text-slate-450 dark:text-slate-650 font-mono font-semibold text-xs select-none">-</span>
                    )}
                  </div>
                )}
              </div>
